...
/**
 * A <tt>Mixer</tt> is an factory that when used in a {@link Mixin} expects to
 * receive the mixin to produce the actual implementation. This can also be used
 * to store the mixing and invoke other methods through the mixin. In that case
 * care must be taken if the object is used in several mixins.
 * 
 * <pre>
